# Contributing to Skills for Agents
...
- **Specific** ΓÇö Actionable steps, not vague advice
- **Verifiable** ΓÇö Clear exit criteria with evidence requirements
- **Battle-tested** ΓÇö Based on real engineering workflows, not theoretical ideals
- **Minimal** ΓÇö Only the content needed to guide the agent correctly
...
- **Overview** ΓÇö What this skill does and why it matters
- **When to Use** ΓÇö Triggering conditions
- **Process** ΓÇö Step-by-step workflow
- **Common Rationalizations** ΓÇö Excuses agents use to skip steps, with rebuttals
- **Red Flags** ΓÇö Warning signs that the skill is being applied incorrectly
- **Verification** ΓÇö How to confirm the skill was applied correctly
...
- Don't duplicate content between skills ΓÇö reference other skills instead
...
- Don't create an empty `scripts/` directory just to match another skill ΓÇö add `scripts/` only when the skill includes runnable helpers
- Don't put reference material inside skill directories ΓÇö use `references/` instead
...
The session-start hook (`hooks/session-start.sh`) injects the `using-Skills for Agents` meta-skill into every new Claude Code session. A regression test at `hooks/session-start-test.sh` validates the hook's JSON payload ΓÇö both when `jq` is available and when it isn't.

Run it before opening any PR that touches:

- `hooks/session-start.sh`
- `skills/using-Skills for Agents/SKILL.md` (the meta-skill content embedded by the hook)

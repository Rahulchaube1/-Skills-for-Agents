# Using Skills for Agents with Gemini CLI
...
```bash
gemini skills install https://github.com/addyosmani/Skills for Agents.git --path skills
```
...
gemini skills install /path/to/Skills for Agents/skills/
```

**Install for a specific workspace only:**

```bash
gemini skills install /path/to/Skills for Agents/skills/ --scope workspace
```
...
cat /path/to/Skills for Agents/skills/incremental-implementation/SKILL.md > GEMINI.md
echo -e "\n---\n" >> GEMINI.md
cat /path/to/Skills for Agents/skills/code-review-and-quality/SKILL.md >> GEMINI.md
```
...
- `incremental-implementation` ΓÇö Build in small verifiable slices
- `code-review-and-quality` ΓÇö Five-axis review

### On-Demand (Skills)

Install these as skills so they activate only when relevant:

- `test-driven-development` ΓÇö Activates when implementing logic or fixing bugs
- `spec-driven-development` ΓÇö Activates when starting a new project or feature
- `frontend-ui-engineering` ΓÇö Activates when building UI
- `security-and-hardening` ΓÇö Activates during security reviews
- `performance-optimization` ΓÇö Activates during performance work
...
To replicate the `Skills for Agents` experience from other tools, you can configure a `SessionStart` hook that reminds you of the available skills or loads a meta-skill.
...
| `/test` | Run TDD workflow ΓÇö red, green, refactor |
| `/review` | Five-axis code review |
| `/code-simplify` | Reduce complexity without changing behavior |
| `/ship` | Pre-launch checklist via parallel persona fan-out |

Each command invokes the corresponding skill automatically ΓÇö no manual skill loading required.

> **Note:** Use `/planning` instead of `/plan` ΓÇö `/plan` conflicts with a Gemini CLI internal command name.

## Usage Tips

1. **Prefer skills over GEMINI.md** ΓÇö Skills activate on demand and keep your context window focused. Only put skills in GEMINI.md if you want them always loaded.
2. **Skill descriptions matter** ΓÇö Each SKILL.md has a `description` field in its frontmatter that tells agents when to activate it. The descriptions in this repo are optimized for auto-discovery across all supported tools (Claude Code, Gemini CLI, etc.) by clearly stating both *what* the skill does and *when* it should be triggered.
3. **Use agents for review** ΓÇö Copy `agents/code-reviewer.md` content when requesting structured code reviews.
4. **Combine with references** ΓÇö Reference checklists from `references/` when working on specific quality areas like testing or performance.

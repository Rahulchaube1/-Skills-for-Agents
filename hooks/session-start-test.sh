...
if (hasJq) {
  ...
  if (!payload.message.includes('Skills for Agents loaded.')) {
    throw new Error('message is missing startup preface');
  }

  if (!payload.message.includes('# Using Skills for Agents')) {
    throw new Error('message is missing using-Skills for Agents content');
  }
} else {
